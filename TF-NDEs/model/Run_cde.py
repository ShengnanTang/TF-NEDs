...
args.add_argument('--Dm', default=config['model']['Dm'], type=int)
args.add_argument('--wave_exp_num', default=config['model']['wave_exp_num'], type=int)
...
args.add_argument('--dec_level', default=1, type=int)
...
#config log path
save_name = beijing_now.strftime("%m-%d-%Hh%Mm") + args.comment + "_" + args.dataset + "_" + args.model + "_" + args.model_type + \
            "_" + "embed{" + str(args.embed_dim) + "}" + \
            "hid{" + str(args.hid_dim) + "}" + \
            "hidhid{" + str(args.hid_hid_dim) + "}" + \
            "lyrs{" + str(args.num_layers) + "}" + \
            "lr{" + str(args.lr_init) + "}" + \
...
elif args.model_type=='type_custom':
    model, vector_field_f,vector_field_g = make_model(args)
else:
...
params_to_skip = ["Ldwt.forward_wavelet_layers.0.low_row.0"]
...
for name, p in model.named_parameters():
    ...
    if p.dim() > 1:
        ...
        nn.init.uniform_(p)
print(model.Ldwt.forward_wavelet_layers[0].low_row[0])
...
if args.mode == 'train':
    ...
    model.load_state_dict(torch.load('/root/TSNWORKSPACE-1/stgncde723f_custom/runs/PEMSD8/07-24-15h35m_PEMSD8_GCDE_type_custom_embed{2}hid{96}hidhid{128}lyrs{2}lr{0.001}wd{0.001}/best_model.pth'))
    print("Load saved model")
    trainer.test(model, trainer.args, test_loader, scaler, trainer.logger, path=None,times=times)
else:
    raise ValueError
