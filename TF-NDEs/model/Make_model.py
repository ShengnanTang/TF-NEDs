...
def make_model(args):
    if args.model_type == 'type1':
        ...
        model = NeuralGCDE(args, func_f=vector_field_f, func_g=vector_field_g, input_channels=args.input_dim, hidden_channels=args.hid_dim,
                                        ...
                                        device=args.device, atol=1e-6, rtol=1e-6, solver=args.solver)
        return model, vector_field_f, vector_field_g
    
    if args.model_type == 'type_custom':

        vector_field_f = FinalTanh_f_custom(input_channels=args.input_dim, hidden_channels=args.hid_dim,
                                            hidden_hidden_channels=args.hid_hid_dim,
                                            num_hidden_layers=args.num_layers, t_total=args.lag)
