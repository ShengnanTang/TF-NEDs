...
class VectorFieldGDE_dev(torch.nn.Module):
    def __init__(self, dX_dt,func_f, func_g,M,mlp,yh_spline):
        """Defines a controlled vector field.
    ...
    def __call__(self, t, hz):
        ...
        yh_derivate = self.yh_spline.derivative(t)
        yh_evaluate = self.yh_spline.evaluate(t)

        yh_total = torch.cat((yh_derivate,yh_evaluate),dim=-1)
        
        h = hz[0] # h: torch.Size([64, 207, 32])
        z = hz[1] # z: torch.Size([64, 207, 32])

        if len(inspect.signature(self.func_f.forward).parameters) == 2:
            vector_field_f = self.func_f(h,t)
        else:
        ...
        attention_weights = torch.softmax((torch.matmul(self.M.unsqueeze(0).unsqueeze(0), ((yh_total).unsqueeze(-1)))), dim=2)
        # control_gradient = control_gradient
        yh_control = torch.matmul(self.M.T.unsqueeze(0).unsqueeze(0), attention_weights).squeeze(-1) 

        control_gradient_combine = torch.cat((control_gradient, yh_control),dim=-1)
        control_gradient_combine = self.mlp(control_gradient_combine)
        
        control_gradient = 0.5*control_gradient_combine + 0.5*control_gradient
...
def cdeint_gde_dev(dX_dt, h0, z0,func_f, func_g, t,M,mlp,yh_spline,adjoint=False, **kwargs):
    r"""Solves a system of controlled differential equations.
    ...
    odeint = torchdiffeq.odeint_adjoint if adjoint else torchdiffeq.odeint
    vector_field = VectorFieldGDE_dev(dX_dt=dX_dt,func_f=func_f, func_g=func_g,M=M,mlp=mlp,yh_spline=yh_spline)
    init0 = (h0,z0)
